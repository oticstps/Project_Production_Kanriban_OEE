...
// Mapping line produksi
const linePartData = {
  'COMMON RAIL 1': ['634F', 'E494', 'E694', 'H6-A', 'P3263','P3267', 'MDE8'],
  'COMMON RAIL 2': ['902FA-PTJ'],
  'COMMON RAIL 3': ['JDE20', '4N13','RT56', 'VM', 'VM USA'],
  'COMMON RAIL 4': ['4N16-B', 'MDE5'],
  'COMMON RAIL 5': ['4J', 'N4','N6', '902F','RT50', '902F E', '902F E-SGI167, ASHOKN4', 'RT-50'],
  'COMMON RAIL 6': ['RZ4E'],
  'COMMON RAIL 7': ['CR22','4N15B', '4N15', 'S320', 'YD25', 'YD2K3',  'S320', '4N15', 'YD2K3'],
  'COMMON RAIL 8': ['902fc', '1KD', '415F'],
  'COMMON RAIL 9': ['902F-B', 'GDB'],
  'COMMON RAIL 10': ['902F G', 'RG01', 'RG01 B'],
  'COMMON RAIL 11': ['19MY', 'RZ4E', '902FD'],
  'COMMON RAIL 12': ['902F-F', 'ES01B', 'ES01']
};

// Mapping produk ke tabel filtered
const productTableMapFiltered = {};
Object.keys(linePartData).forEach(line => {
  const products = linePartData[line];
  const tableName = line.toLowerCase().replace(/\s+/g, '_');
  products.forEach(product => {
    productTableMapFiltered[product] = `${tableName}_filtered`;
  });
});

// Mapping produk ke tabel hourly
const productTableMapHourly = {};
Object.keys(linePartData).forEach(line => {
  const products = linePartData[line];
  const tableName = line.toLowerCase().replace(/\s+/g, '_');
  products.forEach(product => {
    productTableMapHourly[product] = `${tableName}_hourly`;
  });
});
...
function syncDataByProduct(nameProduct) {
  ...
  return new Promise((resolve, reject) => {
    ...
    db.query(selectQuery, [nameProduct], (err, results) => {
      ...
      const existingQuery = `
        SELECT idPrimary, name_product, created_at FROM ?? 
        WHERE name_product = ?
      `;

      db.query(existingQuery, [tableName, nameProduct], (err, existingResults) => {
        if (err) {
...
function getLastProcessedTimestamp(nameProduct) {
  return new Promise((resolve, reject) => {
    ...
    if (!tableName) {
      return reject(new Error(`Tidak ada mapping tabel hourly untuk produk: ${nameProduct}`));
    }
    ...
    db.query(query, [tableName, nameProduct], (err, results) => {
      if (err) {
...
function processHourlyDataByProduct(nameProduct) {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // Query untuk mengambil data dari tabel filtered (hanya data baru)
      let selectQuery;
      let queryParams;
      ...
      if (lastProcessedTimestamp) {
        selectQuery = `
          ...
          ORDER BY created_at ASC
        `;
        queryParams = [productTableMapFiltered[nameProduct], nameProduct, lastProcessedTimestamp];
      } else {
        selectQuery = `
          SELECT * FROM ?? 
          WHERE name_product = ?
          ORDER BY created_at ASC
        `;
        queryParams = [productTableMapFiltered[nameProduct], nameProduct];
      }
      ...
      db.query(selectQuery, queryParams, (err, results) => {
        ...
        const insertPromises = scheduleDataWithDelta.map(row => {
          return new Promise((res, rej) => {
            ...
            db.query(checkQuery, [tableName, row.name_product, row.line_id, scheduleStart, scheduleEnd], (err, checkResult) => {
              if (err) {
              ...
              db.query(insertQuery, [tableName, ...values], (err) => {
                if (err) {
