...
const CommonRail8Filtered = {
  // Get all data
  getAll: async () => {
    try {
      const [rows] = await pool.query('SELECT * FROM common_rail_8_filtered ORDER BY created_at DESC');
      return rows;
    } catch (error) {
      throw error;
    }
  ...
  getByDateTimeRange: async (startDateTime, endDateTime) => {
    try {
      const [rows] = await pool.query(
        'SELECT * FROM common_rail_8_filtered WHERE created_at BETWEEN ? AND ? ORDER BY created_at DESC',
  ...
  getByDate: async (date) => {
    try {
      const [rows] = await pool.query(
        'SELECT * FROM common_rail_8_filtered WHERE DATE(created_at) = ? ORDER BY created_at DESC',
        [date]
      );
      return rows;
    } catch (error) {
  ...
  getByHourRange: async (date, startHour, endHour) => {
    try {
      const [rows] = await pool.query(
        'SELECT * FROM common_rail_8_filtered WHERE DATE(created_at) = ? AND TIME(created_at) BETWEEN ? AND ? ORDER BY created_at DESC',
  ...
  getByExactDateTime: async (dateTime) => {
    try {
      const [rows] = await pool.query(
        'SELECT * FROM common_rail_8_filtered WHERE created_at = ? ORDER BY created_at DESC',
        [dateTime]
      );
      return rows;
    } catch (error) {
  ...
  getByCustomFilter: async (filters) => {
    try {
      let query = 'SELECT * FROM common_rail_8_filtered WHERE 1=1';
      const params = [];
  ...
  getWithPagination: async (page = 1, limit = 50, filters = {}) => {
    try {
      let query = 'SELECT * FROM common_rail_8_filtered WHERE 1=1';
      let countQuery = 'SELECT COUNT(*) as total FROM common_rail_8_filtered WHERE 1=1';
      const params = [];
      const countParams = [];
  ...
  getByMultipleDaysTimeRange: async (startDate, endDate, startTime, endTime) => {
    try {
      const [rows] = await pool.query(`
        SELECT * FROM common_rail_8_filtered 
        WHERE 
...
module.exports = CommonRail8Filtered;