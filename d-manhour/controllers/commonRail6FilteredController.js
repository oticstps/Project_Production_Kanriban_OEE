const CommonRail6Filtered = require('../models/commonRail6FilteredModel');
...
const commonRail6FilteredController = {
  // Get all data
  getAll: async (req, res) => {
    try {
      const data = await CommonRail6Filtered.getAll();
      res.json({
        status: 'success',
        data: data,
        total: data.length,
      ...
      console.error('Error fetching common rail 6 filtered data:', error);
      res.status(500).json({
        status: 'error',
  ...
  getByDateTimeRange: async (req, res) => {
    try {
      ...
      const data = await CommonRail6Filtered.getByDateTimeRange(startDateTime, endDateTime);
  ...
  getByDate: async (req, res) => {
    try {
      ...
      const data = await CommonRail6Filtered.getByDate(date);
  ...
  getByTimeRange: async (req, res) => {
    try {
      ...
      const data = await CommonRail6Filtered.getByTimeRange(date, startTime, endTime);
  ...
  getByMultipleDaysTimeRange: async (req, res) => {
    try {
      ...
      const data = await CommonRail6Filtered.getByMultipleDaysTimeRange(startDate, endDate, startTime, endTime);
  ...
  getWithFilter: async (req, res) => {
    try {
      ...
      const data = await CommonRail6Filtered.getByCustomFilter(filters);
  ...
  getWithPagination: async (req, res) => {
    try {
      ...
      const result = await CommonRail6Filtered.getWithPagination(page, limit, filters);
  ...
  getHourlyStats: async (req, res) => {
    try {
      ...
      const stats = await CommonRail6Filtered.getHourlyStats(date);
...
module.exports = commonRail6FilteredController;