const CommonRail11Filtered = require('../models/commonRail11FilteredModel');
...
const commonRail11FilteredController = {
  // Get all data
  getAll: async (req, res) => {
    try {
      const data = await CommonRail11Filtered.getAll();
      res.json({
        status: 'success',
        data: data,
        total: data.length,
      ...
      console.error('Error fetching common rail 11 filtered data:', error);
      res.status(500).json({
        status: 'error',
  ...
  getByDateTimeRange: async (req, res) => {
    try {
      ...
      const data = await CommonRail11Filtered.getByDateTimeRange(startDateTime, endDateTime);
  ...
  getByDate: async (req, res) => {
    try {
      ...
      const data = await CommonRail11Filtered.getByDate(date);
  ...
  getByTimeRange: async (req, res) => {
    try {
      ...
      const data = await CommonRail11Filtered.getByTimeRange(date, startTime, endTime);
  ...
  getByMultipleDaysTimeRange: async (req, res) => {
    try {
      ...
      const data = await CommonRail11Filtered.getByMultipleDaysTimeRange(startDate, endDate, startTime, endTime);
  ...
  getWithFilter: async (req, res) => {
    try {
      ...
      const data = await CommonRail11Filtered.getByCustomFilter(filters);
  ...
  getWithPagination: async (req, res) => {
    try {
      ...
      const result = await CommonRail11Filtered.getWithPagination(page, limit, filters);
  ...
  getHourlyStats: async (req, res) => {
    try {
      ...
      const stats = await CommonRail11Filtered.getHourlyStats(date);
...
module.exports = commonRail11FilteredController;