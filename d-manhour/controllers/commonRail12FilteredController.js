const CommonRail12Filtered = require('../models/commonRail12FilteredModel');
...
const commonRail12FilteredController = {
  // Get all data
  getAll: async (req, res) => {
    try {
      const data = await CommonRail12Filtered.getAll();
      res.json({
        status: 'success',
        data: data,
        total: data.length,
      ...
      console.error('Error fetching common rail 12 filtered data:', error);
      res.status(500).json({
        status: 'error',
  ...
  getByDateTimeRange: async (req, res) => {
    try {
      ...
      const data = await CommonRail12Filtered.getByDateTimeRange(startDateTime, endDateTime);
  ...
  getByDate: async (req, res) => {
    try {
      ...
      const data = await CommonRail12Filtered.getByDate(date);
  ...
  getByTimeRange: async (req, res) => {
    try {
      ...
      const data = await CommonRail12Filtered.getByTimeRange(date, startTime, endTime);
  ...
  getByMultipleDaysTimeRange: async (req, res) => {
    try {
      ...
      const data = await CommonRail12Filtered.getByMultipleDaysTimeRange(startDate, endDate, startTime, endTime);
  ...
  getWithFilter: async (req, res) => {
    try {
      ...
      const data = await CommonRail12Filtered.getByCustomFilter(filters);
  ...
  getWithPagination: async (req, res) => {
    try {
      ...
      const result = await CommonRail12Filtered.getWithPagination(page, limit, filters);
  ...
  getHourlyStats: async (req, res) => {
    try {
      ...
      const stats = await CommonRail12Filtered.getHourlyStats(date);
...
module.exports = commonRail12FilteredController;