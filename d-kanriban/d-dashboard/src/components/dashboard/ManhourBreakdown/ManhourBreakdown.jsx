...
const ManhourBreakdown = () => {
  ...
  const uniqueProductNames = [...new Set(
    data
      .filter(item =>
        (pg === '' || item.pg === pg) &&
        (lineName === '' || item.line_name === lineName) // Filter berdasarkan Line Name jika dipilih
      )
  ...
  // Perbaikan: Fungsi groupByDateAndShift untuk memastikan semua tanggal dan shift ada
  const groupByDateAndShift = (items) => {
    const grouped = {};
    const tanggalBulan = generateTanggalBulan();

    // Inisialisasi semua tanggal dan shift dengan array kosong
    ...
    // Masukkan item ke dalam grup berdasarkan tanggal dan shift jika statusnya STOP
    items.forEach(item => {
      if (item.status === 'STOP') {
        const date = new Date(item.start_time);
  ...
  const exportToExcel = async () => {
    ...
    // Definisikan header - tanpa Line Name dan PG
    const headers = [
      'Tanggal',
      'Shift',
      'Σ Produksi (pcs) A',
    ...
    tanggalBulan.forEach(day => {
      ...
      if (dayData) {
          ...
          if (shiftData && shiftData.length > 0) {
            shiftData.forEach((item, index) => {
              ...
              worksheet.addRow([
                ...
                loadingTimeMenit,
                item.manpower_help,
                item.loading_time_manpower_help,
                item.machine_fault_preq,
                item.machine_fault_duration,
                item.dangae,
                item.quality_check,
                item.another,
                item.kaizen,
                item.five_s,
                Math.round(rincianInLine),
                Math.round(rincianBantuan),
                Math.round(rincian5S),
                Math.round(sigmaManhour),
                Math.round(manhourPerPcs),
                Math.round(pe)
              ]);
            });
          } else {
            // Jika tidak ada data untuk shift ini, tambahkan baris kosong
            worksheet.addRow([
              day,
              shiftName,
              '-',
        ...
        // Jika tidak ada data untuk hari itu (seharusnya tidak terjadi karena inisialisasi), tambahkan baris kosong untuk kedua shift
        ['Shift 1', 'Shift 2'].forEach(shiftName => {
          worksheet.addRow([
            day,
            shiftName,
  ...
  const handleOpenCreateModal = () => {
    ...
    setFormData({
      ...
      line_name: lineName, // Isi otomatis dengan lineName yang dipilih
    ...
    <Layout>
        <div className="p-2">
            <div className="flex justify-between items-center mb-3">
                ...
                <p className='italic font-bold underline text-blue-900'>CR5 in progress maintenance</p>
            ...
            {/* Info Card Section - Include Line Name and PG as cards */}
            <div className="mb-3 p-2 bg-white-50 border border-blue-200 rounded-lg">
                <div className="grid grid-cols-1 md:grid-cols-5 gap-2">
                    ...
                    <div className="p-2 bg-white border border-gray-200 rounded-md text-center">
                        ...
                        <p className="text-sm font-medium text-gray-900">{String(infoCardData.cycle_time / 10)}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow overflow-auto max-h-[65vh]">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50 sticky top-0 z-10">
                    <tr>
                      <th rowSpan="2" className="px-1.5 py-1.5 text-xs font-medium text-gray-900 uppercase tracking-wider border w-12">TGL</th>
                      <th rowSpan="2" className="px-1.5 py-1.5 text-xs font-medium text-gray-900 uppercase tracking-wider border w-12">Shift</th>
                      <th rowSpan="2" className="px-1.5 py-1.5 text-xs font-medium text-gray-900 uppercase tracking-wider border w-16 bg-teal-400">Σ Produksi (pcs) A</th>
                      ...
                      <th rowSpan="2" className="px-1.5 py-1.5 text-xs font-medium text-gray-900 uppercase tracking-wider border w-16 bg-teal-400">LT (Menit) ③</th>
                      ...
                      <th rowSpan="2" className="px-1.5 py-1.5 text-xs font-medium text-gray-900 uppercase tracking-wider border w-16">LT Opr Bantuan (Menit) ⑤</th>
                      <th colSpan="2" className="px-1.5 py-1.5 text-xs font-medium text-gray-900 uppercase tracking-wider text-center border w-16">Perbaikan Mesin (Menit)</th>
                      <th rowSpan="2" className="px-1.5 py-1.5 text-xs font-medium text-gray-900 uppercase tracking-wider border w-12">Dangae</th>
                      <th rowSpan="2" className="px-1.5 py-1.5 text-xs font-medium text-gray-900 uppercase tracking-wider border w-16">Quality Check (Menit)</th>
                      ...
                      <th rowSpan="2" className="px-1.5 py-1.5 text-xs font-medium text-gray-900 uppercase tracking-wider border w-16">Kaizen (menit) Out LT</th>
                      <th rowSpan="2" className="px-1.5 py-1.5 text-xs font-medium text-gray-900 uppercase tracking-wider border w-12">5S (menit) ⑥</th>
                      <th colSpan="3" className="px-1.5 py-1.5 text-xs font-medium text-gray-900 uppercase tracking-wider text-center border w-16">Rincian Manhour (menit)</th>
                      <th rowSpan="2" className="px-1.5 py-1.5 text-xs font-medium text-gray-900 uppercase tracking-wider border w-16 bg-orange-200">Σ Manhour (manmnt)</th>
                      <th rowSpan="2" className="px-1.5 py-1.5 text-xs font-medium text-gray-900 uppercase tracking-wider border w-16 bg-orange-200">Manhour (man.menit / pcs)</th>
                      <th rowSpan="2" className="px-1.5 py-1.5 text-xs font-medium text-gray-900 uppercase tracking-wider border w-12">PE (%)</th>
                      <th rowSpan="2" className="px-1.5 py-1.5 text-xs font-medium text-gray-900 uppercase tracking-wider border w-16 bg-purple-200">Action</th>
                    </tr>
                    <tr>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                      {generateTanggalBulan().map(day => {
                          ...
                          return shifts.map((shiftName, shiftIndex) => {
                              const shiftItems = dayData?.[shiftName] || [];
                              const hasData = shiftItems.length > 0;
                              const item = hasData ? shiftItems[0] : null; // Ambil item pertama jika ada data

                              // --- Perhitungan Manhour dan PE untuk Tabel ---
                              let opInLine, durationMenit, loadingTimeMenit, oprBantuan, loadingTimeOprBantuan, fiveSMenit, linegaiMenit, rincianInLine, rincianBantuan, rincian5S, rincianLinegai, sigmaManhour, manhourPerPcs, cycleTimeAsli, cycleTimeInSeconds, pe;
                              if (hasData) {
                                  opInLine = parseFloat(item.manpower) || 0;
                                  durationMenit = Math.round(item.duration_minutes || 0);
                                  loadingTimeMenit = Math.round(parseFloat(item.loading_time_calc) || 0);
                                  oprBantuan = parseFloat(item.manpower_help) || 0;
                                  loadingTimeOprBantuan = parseFloat(item.loading_time_manpower_help) || 0;
                                  fiveSMenit = parseFloat(item.five_s) || 0;
                                  linegaiMenit = parseFloat(item.another) || 0;

                                  rincianInLine = opInLine * loadingTimeMenit;
                                  rincianBantuan = oprBantuan * loadingTimeOprBantuan;
                                  rincian5S = parseFloat(item.manhour_five_s) || 0;
                                  rincianLinegai = linegaiMenit;

                                  sigmaManhour = rincianInLine + rincianBantuan + fiveSMenit + rincian5S + rincianLinegai;

                                  const produksiPcs = parseInt(item.total_produced) || 0;
                                  manhourPerPcs = produksiPcs > 0 ? sigmaManhour / produksiPcs : 0;

                                  cycleTimeAsli = parseFloat(item.cycle_time) || 0;
                                  cycleTimeInSeconds = cycleTimeAsli / 10;
                                  pe = loadingTimeMenit > 0 ? ((produksiPcs * cycleTimeInSeconds) / 60 / loadingTimeMenit) * 100 : 0;
                              } else {
                                  // Jika tidak ada data, gunakan placeholder
                                  opInLine = durationMenit = loadingTimeMenit = oprBantuan = loadingTimeOprBantuan = fiveSMenit = linegaiMenit = rincianInLine = rincianBantuan = rincian5S = rincianLinegai = sigmaManhour = manhourPerPcs = cycleTimeAsli = cycleTimeInSeconds = pe = 0;
                              }

                              const rowClass = shiftIndex === 0 ? "border-t-2 border-gray-300" : "";
                              const rowBgClass = hasData ? "" : "bg-red-50"; // Warna latar belakang jika tidak ada data

                              return (
                                  <tr key={`${dateKey}-${shiftName}`} className={`${rowClass} ${rowBgClass}`}>
                                      <td className="px-1.5 py-1.5 whitespace-nowrap text-xs font-medium text-gray-900 border text-center">{day}</td>
                                      <td className="px-1.5 py-1.5 whitespace-nowrap text-xs font-medium text-gray-900 border text-center">{shiftName}</td>
                                      <td className="px-1.5 py-1.5 whitespace-nowrap text-xs text-gray-800 border text-center bg-teal-100">{hasData ? item.total_produced : '-'}</td>
                                      <td className="px-1.5 py-1.5 whitespace-nowrap text-xs text-gray-800 border text-center">{hasData ? item.manpower : '-'}</td>
                                      <td className="px-1.5 py-1.5 whitespace-nowrap text-xs text-gray-800 border text-center bg-teal-100">{hasData ? loadingTimeMenit : '-'}</td>
                                      <td className="px-1.5 py-1.5 whitespace-nowrap text-xs text-gray-800 border text-center">{hasData ? item.manpower_help : '-'}</td>
                                      <td className="px-1.5 py-1.5 whitespace-nowrap text-xs text-gray-800 border text-center">{hasData ? item.loading_time_manpower_help : '-'}</td>
                                      <td className="px-1.5 py-1.5 whitespace-nowrap text-xs text-gray-800 border text-center">{hasData ? item.machine_fault_preq : '-'}</td>
                                      <td className="px-1.5 py-1.5 whitespace-nowrap text-xs text-gray-800 border text-center">{hasData ? item.machine_fault_duration : '-'}</td>
                                      <td className="px-1.5 py-1.5 whitespace-nowrap text-xs text-gray-800 border text-center">{hasData ? item.dangae : '-'}</td>
                                      <td className="px-1.5 py-1.5 whitespace-nowrap text-xs text-gray-800 border text-center">{hasData ? item.quality_check : '-'}</td>
                                      <td className="px-1.5 py-1.5 whitespace-nowrap text-xs text-gray-800 border text-center">{hasData ? item.another : '-'}</td>
                                      <td className="px-1.5 py-1.5 whitespace-nowrap text-xs text-gray-800 border text-center">{hasData ? item.kaizen : '10'}</td>
                                      <td className="px-1.5 py-1.5 whitespace-nowrap text-xs text-gray-800 border text-center">{hasData ? item.five_s : '10'}</td>
                                      <td className="px-1.5 py-1.5 whitespace-nowrap text-xs text-gray-800 border text-center">{hasData ? Math.round(rincianInLine) : '-'}</td>
                                      <td className="px-1.5 py-1.5 whitespace-nowrap text-xs text-gray-800 border text-center">{hasData ? Math.round(rincianBantuan) : '-'}</td>
                                      <td className="px-1.5 py-1.5 whitespace-nowrap text-xs text-gray-800 border text-center">{hasData ? Math.round(rincian5S) : '-'}</td>
                                      <td className="px-1.5 py-1.5 whitespace-nowrap text-xs text-gray-800 border text-center">{hasData ? Math.round(sigmaManhour) : '-'}</td>
                                      <td className="px-1.5 py-1.5 whitespace-nowrap text-xs text-gray-800 border text-center">{hasData ? Math.round(manhourPerPcs) : '-'}</td>
                                      <td className="px-1.5 py-1.5 whitespace-nowrap text-xs text-gray-800 border text-center">{hasData ? Math.round(pe) : '-'}</td>
                                      <td className="px-1.5 py-1.5 whitespace-nowrap text-xs text-gray-800 border text-center">
                                          {hasData ? (
                                              <>
                                                  <button onClick={() => handleOpenEditModal(item)} className="text-blue-600 hover:text-blue-900 mr-2">Edit</button>
                                                  <button onClick={() => handleDelete(item.idPrimary)} className="text-red-600 hover:text-red-900">Delete</button>
                                              </>
                                          ) : (
                                              <button onClick={handleOpenCreateModal} className="text-blue-600 hover:text-blue-900 mr-2">Add</button>
                                          )}
                                      </td>
                                  </tr>
                              );
