const CommonRail7Filtered = require('../models/commonRail7FilteredModel');
...
const commonRail7FilteredController = {
  // Get all data
  getAll: async (req, res) => {
    try {
      const data = await CommonRail7Filtered.getAll();
      res.json({
        status: 'success',
        data: data,
        total: data.length,
      ...
      console.error('Error fetching common rail 7 filtered data:', error);
      res.status(500).json({
        status: 'error',
  ...
  getByDateTimeRange: async (req, res) => {
    try {
      ...
      const data = await CommonRail7Filtered.getByDateTimeRange(startDateTime, endDateTime);
  ...
  getByDate: async (req, res) => {
    try {
      ...
      const data = await CommonRail7Filtered.getByDate(date);
  ...
  getByTimeRange: async (req, res) => {
    try {
      ...
      const data = await CommonRail7Filtered.getByTimeRange(date, startTime, endTime);
  ...
  getByMultipleDaysTimeRange: async (req, res) => {
    try {
      ...
      const data = await CommonRail7Filtered.getByMultipleDaysTimeRange(startDate, endDate, startTime, endTime);
  ...
  getWithFilter: async (req, res) => {
    try {
      ...
      const data = await CommonRail7Filtered.getByCustomFilter(filters);
  ...
  getWithPagination: async (req, res) => {
    try {
      ...
      const result = await CommonRail7Filtered.getWithPagination(page, limit, filters);
  ...
  getHourlyStats: async (req, res) => {
    try {
      ...
      const stats = await CommonRail7Filtered.getHourlyStats(date);
...
module.exports = commonRail7FilteredController;