const CommonRail1Filtered = require('../models/commonRail1FilteredModel');
...
const commonRail1FilteredController = {
  // Get all data
  getAll: async (req, res) => {
    try {
      const data = await CommonRail1Filtered.getAll();
      res.json({
        status: 'success',
        data: data,
        total: data.length,
      ...
      console.error('Error fetching common rail 1 filtered data:', error);
      res.status(500).json({
        status: 'error',
  ...
  getByDateTimeRange: async (req, res) => {
    try {
      ...
      const data = await CommonRail1Filtered.getByDateTimeRange(startDateTime, endDateTime);
  ...
  getByDate: async (req, res) => {
    try {
      ...
      const data = await CommonRail1Filtered.getByDate(date);
  ...
  getByTimeRange: async (req, res) => {
    try {
      ...
      const data = await CommonRail1Filtered.getByTimeRange(date, startTime, endTime);
  ...
  getByMultipleDaysTimeRange: async (req, res) => {
    try {
      ...
      const data = await CommonRail1Filtered.getByMultipleDaysTimeRange(startDate, endDate, startTime, endTime);
  ...
  getWithFilter: async (req, res) => {
    try {
      ...
      const data = await CommonRail1Filtered.getByCustomFilter(filters);
  ...
  getWithPagination: async (req, res) => {
    try {
      ...
      const result = await CommonRail1Filtered.getWithPagination(page, limit, filters);
  ...
  getHourlyStats: async (req, res) => {
    try {
      ...
      const stats = await CommonRail1Filtered.getHourlyStats(date);
...
module.exports = commonRail1FilteredController;